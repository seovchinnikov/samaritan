...
$(document).ready(function(){
    ...
    //randomTimePhrase();
	randomTimewhatAreYourCommandsPhrase();
})

var whatAreYourCommands = function()
{
	executeSamaritan("What are your commands ?", startDictation);
...
var randomTimewhatAreYourCommandsPhrase = function()
{
    ...
    $State.randomTimer = setTimeout( whatAreYourCommands, randomTime);
}
...
var executeSamaritan = function(phrase, after=null)
{
    if ($State.isText)
        return;
...
var go_to_search = function(query){
	window.location = "https://www.google.ru/search?q="+encodeURIComponent(query);
	return true;
}
function startDictation() {
	...
	if (window.hasOwnProperty('webkitSpeechRecognition')) {
		...
		recognition.onresult = function(e) {
			var go_to = function() {
				go_to_search(e.results[0][0].transcript)
			};
			var time_out_and_go = function() {
